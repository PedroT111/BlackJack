...
const reporte1 = async (req, res) => {
    try {
        ...
        if (idUsuario === null) {
          ...
          const totalJugadas = await Jugada.count({where: {UsuarioId: idUsuario}});
          const gano = await Jugada.count({where:{UsuarioId: idUsuario, resultado: 1}});
          const perdio = await Jugada.count({where:{UsuarioId: idUsuario, resultado: -1}});
          const empato = await Jugada.count({where:{UsuarioId: idUsuario, resultado: 0}});
...
const reporte2 =async (req, res) => {
  try {
    ...
    if (idUsuario === null) {
      ...
      const totalJugadasGanadas = await Jugada.count({where: {UsuarioId: idUsuario, resultado: 1}});
      const blackjack = await Jugada.count({where:{UsuarioId: idUsuario, blackjack: 1, resultado: 1}});
      const ganoSinBj = await Jugada.count({where:{UsuarioId: idUsuario, blackjack: 0, resultado: 1}});
